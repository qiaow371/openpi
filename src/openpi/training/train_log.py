...
# Baseline matches configs/train_pi05.yaml numeric defaults (strings/bools ignored).
# getter -> (actual value from TrainConfig); core keys skipped in the diff section.
...
_EXP_NAME_NUMERIC_FIELDS: list[_ExpField] = [
    ...
    ("num_workers", "nw", lambda c: _cfg_get(c, "num_workers"), 2),
    ...
    ("save_full_every_epochs", "sfe", lambda c: _cfg_get(c, "save_full_every_epochs"), 20),
    ("save_interval", "si", lambda c: _cfg_get(c, "save_interval"), 1000),
    ("save_full_interval", "sfi", lambda c: _cfg_get(c, "save_full_interval"), None),
...
def collect_numeric_exp_name_diffs(config: Any) -> list[tuple[str, str]]:
    ...
    for key, abbr, getter, default in _EXP_NAME_NUMERIC_FIELDS:
        ...
        if key == "save_every_epochs" and not use_save_every:
            continue
        if key == "save_full_interval":
            continue
        if key == "save_full_every_epochs" and _cfg_get(config, "save_full_every_epochs") is None:
            continue
...
def _cap_dirname(parts: list[str], *, max_bytes: int = 240) -> str:
    ...
    # policy, chunk, lr, epochs|steps, ...diffs..., date
    core = parts[:4]
    date = parts[-1]
    kept: list[str] = []
...
def build_exp_name(config: Any, *, now: datetime | None = None) -> str:
    ...
    numeric hyperparameters that differ from ``configs/train_pi05.yaml`` baseline
    (strings and bools ignored). Names are capped near Linux NAME_MAX.
    """
...
def _exp_name_is_missing(exp_name: Any) -> bool:
    ...
    if not text or text.upper() == "MISSING":
        return True
    # tyro: Missing / PropagatingMissingType（大小写不一，统一 upper 判断）
    if "MISSING" in type(exp_name).__name__.upper():
        return True
    return text.lower() in {"auto", "null", "none"}


def resolve_exp_name(config: Any) -> Any:
    """Auto-fill ``exp_name`` like ACT run dirs when YAML leaves it unset/auto.
    ...
    *,
    ...
    payload = {
        "_meta": {
            ...
            "prompt": prompt_meta,
            "trainable": {
                "trainable_modules": getattr(config, "trainable_modules", "all"),
                "stage2_trainable_modules": getattr(config, "stage2_trainable_modules", None),
                "stage2_start_epoch": getattr(config, "stage2_start_epoch", None),
                "stage2_start_step": getattr(config, "stage2_start_step", None),
            },
        },
