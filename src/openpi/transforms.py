...
class PromptFromSubtaskSpans(DataTransformFn):
    ...
    Prefers ``meta/episodes_detailed_task.jsonl`` (InternVLA / AgiBot World).
    Hits replace ``prompt`` with ``action_text``; misses keep the global task.
    """

    # episode -> sorted (start, end, subtask)
    spans: dict[int, tuple[tuple[int, int, str], ...]]

    @classmethod
    def from_jsonl(cls, path: str | pathlib.Path) -> "PromptFromSubtaskSpans":
        """Load AgiBot ``action_config`` jsonl or flat span jsonl."""
        import json
