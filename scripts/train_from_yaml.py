...
def main() -> None:
    ...
    print(f"save_every:  {cfg.save_interval} steps (params-only)")
    if cfg.save_full_interval is None:
        print("save_full:   every save (params + train_state)")
    elif int(cfg.save_full_interval) <= 0:
        print("save_full:   disabled (params-only; resume from last optimizer ckpt)")
    else:
        print(
            f"save_full:   every {cfg.save_full_interval} steps "
            f"+ offset {cfg.save_full_offset} (optimizer, not on params steps)"
        )
