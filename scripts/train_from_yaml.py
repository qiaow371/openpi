...
def main() -> None:
    ...
    print(f"visual_norm:         {getattr(data_cfg, 'visual_norm', 'siglip')}  (RGB+DEPTH)")
    print(f"vector_norm:         {getattr(data_cfg, 'vector_norm', '') or 'quantile'}  (state+action+FT)")
    print(f"max_depth_mm:        {getattr(data_cfg, 'max_depth_mm', 4000.0)}  (DEPTH fallback; train uses depth_mm q01/q99)")
