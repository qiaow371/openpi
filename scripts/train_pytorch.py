...
def train_loop(config: _config.TrainConfig):
    ...
    if config.pytorch_weight_path is not None:
        ...
        disallowed_unexpected = set(unexpected) - allowed_unexpected
        if missing or disallowed_unexpected:
            raise RuntimeError(
                "Incompatible PyTorch checkpoint: "
                f"missing={sorted(missing)}, unexpected={sorted(disallowed_unexpected)}"
            )
        if unexpected:
            logging.info(f"Ignored known tied-weight aliases: {sorted(unexpected)}")
