#!/bin/bash
# 00086 π0.5 训练（4×PPU GPU 1,2,3,4，在 openpi_train 容器内跑）：
#   configs/train_pi05_00086_4gpu_bs384_ah15.yaml  bs384 / action_horizon=15 / 100ep / sv10ep
#   超参沿用 00036 4 卡版配方，prompt 换为 00086 数据集 task。
# 宿主机执行：bash /nvme1n1/openpi/cigai_train/openpi-jiangsuan/scripts/start_train_00086_4gpu_ah15.sh
set -euo pipefail
...
GPUS="${GPUS:-1,2,3,4}"
PORT="${PORT:-29797}"
YAML="configs/train_pi05_00086_4gpu_bs384_ah15.yaml"
LOG="${CONT_LOG_DIR}/train_00086_4gpu_bs384_ah15.log"
...
echo "[log]   容器 ${LOG}  (宿主机 /nvme1n1/openpi/train_00086_4gpu_bs384_ah15.log)"
echo "[ckpt]  ${OPENPI_REPO}/checkpoints/pi05_00086_4gpu_bs384_ah15_20260915"
