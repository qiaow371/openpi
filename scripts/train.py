...
def main(config: _config.TrainConfig):
    ...
    def keep_full_step(step: int) -> bool:
        if _schedule.is_full_save_step(
            int(step),
            config.save_full_interval,
            int(config.save_full_offset or 0),
            start_step=0,
        ):
            return True
        # When optimizer saves are disabled, still keep existing full ckpts
        # (needed for --resume).
        step_dir = config.checkpoint_dir / "checkpoints" / str(int(step))
        return (step_dir / "train_state").exists()
    ...
    checkpoint_manager, resuming = _checkpoints.initialize_checkpoint_dir(
        ...
        should_keep_fn=keep_full_step if config.save_full_interval is not None else None,
    )
    ...
    for step in pbar:
        ...
        params_save = _schedule.is_params_save_step(
            step, config.save_interval, start_step, is_last=is_last
        )
        full_save = _schedule.is_full_save_step(
            step,
            config.save_full_interval,
            int(config.save_full_offset or 0),
            start_step,
        )
        # Last step: keep it params-only unless it already is a staggered full step.
        if is_last and config.save_full_interval is not None:
            params_save = not full_save
        should_save = params_save or full_save
        if should_save:
            include_train_state = full_save if config.save_full_interval is not None else True
            # Never write optimizer and params-only in the same step.
            if full_save:
                params_save = False
